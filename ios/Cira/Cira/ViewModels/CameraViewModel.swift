...
class CameraViewModel: ObservableObject {
    ...
    // MARK: - Voice Recording (Phase 3)
    @Published var showVoiceRecording: Bool = false
    ...
    func setupCamera() async {
        if cameraPermissionGranted {
            ...
            // Session is started inside setupCamera
        }
    }
    ...
    func savePhoto() async throws {
        ...
        try await photoStorageService.saveToLibrary(image: image)
        
        // Create Photo model
        let photo = Photo(
            imageURL: nil, // Will be set after saving file
            dominantColor: dominantColor.toHex() ?? "#000000",
            tags: selectedTags,
