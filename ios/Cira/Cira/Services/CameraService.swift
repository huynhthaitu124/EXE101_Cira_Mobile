...
class CameraService: NSObject, ObservableObject {
    ...
    @Published @MainActor var previewLayer: AVCaptureVideoPreviewLayer?
    @Published @MainActor var isSessionRunning = false
    ...
    func setupCamera(position: AVCaptureDevice.Position = .back) {
        sessionQueue.async { [weak self] in
            ...
            // IMPORTANT: Start session AFTER commitConfiguration
    ...
    func capturePhoto(completion: @escaping (UIImage?) -> Void) {
        sessionQueue.async { [weak self] in
            ...
            // Set orientation (iOS 17+ uses videoRotationAngle)
            if #available(iOS 17.0, *) {
                if videoConnection.isVideoRotationAngleSupported(0) {
                    videoConnection.videoRotationAngle = 0 // Portrait
                }
            } else {
